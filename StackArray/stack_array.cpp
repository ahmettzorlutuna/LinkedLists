...
void push(int a, stack *s)
{
    ...
    if (s->top >= s->size-1) //Dizinin tepe noktası boyutdan bir küçükse.
    {
        int *array2 = (int *)malloc(sizeof(int) * s->size * 2); //Önceki boyutun iki katı kadar yeni bir array.
        for (int i = 0; i < s->size; i++)
        {
            array2[i] = s->array[i]; //Eski arraydeki elemanları kopyaladık.
        }
        int *temp = s-> array;
        s->array = array2;
        free(temp);
